  ...
  (function () {
    var slider = $("#js_date_slider").slider({
      ...
      slide: function(event, ui) {
        $("#js_date_slider_container .js_click:nth-child(" + (7 - ui.value) + ")").trigger('click');
      }
    });
...
var AnalyticsSection = {
  onKeyStats: function (hotKeys) {
    $('#js_top_keys_block').need(1).toggle(hotKeys !== null);
    if (hotKeys == null) {
      return;
    }
  ...
  init: function () {
    ...
    $('#analytics .js_block-expander').live('click', function () {
    ...
    IOCenter.staleness.subscribeValue(function (stale) {
      ...
      $('#analytics .js_staleness-notice')[stale ? 'show' : 'hide']();
    });
    ...
    StatsModel.serverResourcesVisible.subscribeValue(function (visible) {
      $('#analytics')[visible ? 'addClass' : 'removeClass']('dynamic_with_server_resources');
    });
    ...
    $(self.widget).bind('menelaus.graphs-widget.rendered-graphs', function () {
      ...
      $('#analytics .js_current-graph-name').text(graph.title);
      $('#analytics .js_current-graph-desc').text(graph.desc);
    });
    ...
    StatsModel.displayingSpecificStatsCell.subscribeValue(function (displayingSpecificStats) {
      ...
      $('#analytics .js_when-normal-stats').toggle(!displayingSpecificStats);
      $('#analytics .js_when-specific-stats').toggle(displayingSpecificStats);
    });
